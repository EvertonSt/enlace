    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-900">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-300">{t('login.email')}</label>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-300">{t('login.password')}</label>
        ...
        <p className="mt-6 text-center text-xs text-gray-500">
          {t('login.staffOnly')} • {t('login.contactAdmin')}
        </p>
      </motion.div>
    </div>
  );
